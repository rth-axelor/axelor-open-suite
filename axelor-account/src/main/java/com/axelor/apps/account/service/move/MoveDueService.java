...
public class MoveDueService {
  ...
  public List<MoveLine> getInvoiceDue(Invoice invoice, boolean useOthersInvoiceDue)
    ...
    if (useOthersInvoiceDue) {
      othersDebitMoveLines = moveLineToolService.getMoveExcessDueList(false, company, invoice.getPartner(), invoice.getId());
    ...
    debitMoveLines =
            debitMoveLines.stream().distinct().collect(Collectors.toList());
