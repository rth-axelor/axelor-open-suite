...
public class MoveLineToolServiceImpl implements MoveLineToolService {
  ...
  @Override
  public List<MoveLine> getMoveExcessDueList(boolean excessPayment, Company company, Partner partner, Long invoiceId){
    String filter = "";
    if (excessPayment) {
    ...
    filter = filter.concat(" AND self.move.company = :company AND (self.move.statusSelect = :statusAccounted OR self.move.statusSelect = :statusDaybook) "
    ...
    bindings.put("technicalTypesToExclude", Arrays.asList(AccountTypeRepository.TYPE_VIEW, AccountTypeRepository.TYPE_TAX));
    bindings.put("partner", partner);
    bindings.put("invoiceId", invoiceId);

    return moveLineRepository.all()
            .filter(filter)
            .bind(bindings)
            .fetch();
  }
